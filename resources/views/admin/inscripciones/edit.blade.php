<x-app-layout>
    ...
    <div>
        <div class="ui-wrap" style="max-width: 860px;">
            <x-ui.hero :volver="route('admin.inscripciones.index')" volver-texto="Inscripciones" titulo="Editar inscripción" subtitulo="Actualiza el estado o las horas de la inscripción." />

            <div class="bg-white shadow rounded-lg p-6 sm:p-8">
                <form action="{{ route('admin.inscripciones.update', $inscripcione) }}" method="POST" data-confirm-title="¿Guardar los cambios?" data-confirm-text="Se actualizará la información." data-confirm-button="Sí, guardar">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <select name="estudiante_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            ...
                            @foreach ($estudiantes as $estudiante)
                                <option value="{{ $estudiante->id }}" {{ old('estudiante_id', $inscripcione->estudiante_id) == $estudiante->id ? 'selected' : '' }}>
                                    {{ $estudiante->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="proyecto_vinculacion_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            ...
                            @foreach ($proyectos as $proyecto)
                                <option value="{{ $proyecto->id }}" {{ old('proyecto_vinculacion_id', $inscripcione->proyecto_vinculacion_id) == $proyecto->id ? 'selected' : '' }}>
                                    {{ $proyecto->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="fecha_inscripcion" value="{{ old('fecha_inscripcion', $inscripcione->fecha_inscripcion) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="horas_cumplidas" min="0" value="{{ old('horas_cumplidas', $inscripcione->horas_cumplidas) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="estado" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            <option value="activo" {{ old('estado', $inscripcione->estado) == 'activo' ? 'selected' : '' }}>Activo</option>
                            <option value="completado" {{ old('estado', $inscripcione->estado) == 'completado' ? 'selected' : '' }}>Completado</option>
                            <option value="retirado" {{ old('estado', $inscripcione->estado) == 'retirado' ? 'selected' : '' }}>Retirado</option>
                        </select>
