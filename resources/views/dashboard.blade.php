            ...
            <div class="bg-white rounded-lg shadow mb-8">
                ...
                <div class="px-6 py-4 border-t border-gray-200 bg-gray-50 flex items-center justify-between">
                    <div class="text-sm text-gray-600">
                        Mostrando <span class="font-medium">{{ $certificadosPendientes->firstItem() ?? 0 }}</span> a 
                        <span class="font-medium">{{ $certificadosPendientes->lastItem() ?? 0 }}</span> de 
                        <span class="font-medium">{{ $certificadosPendientes->total() }}</span> certificados
                    </div>

                    @if($certificadosPendientes->lastPage() > 1)
                    <div class="flex items-center space-x-2">
                        {{-- Botón Anterior --}}
                        @if($certificadosPendientes->onFirstPage())
                            <button disabled class="px-3 py-2 text-sm font-medium text-gray-400 bg-gray-200 rounded-lg cursor-not-allowed">
                                ← Anterior
                            </button>
                        @else
                            <a href="{{ $certificadosPendientes->previousPageUrl() }}" class="px-3 py-2 text-sm font-medium text-blue-600 bg-white border border-gray-300 rounded-lg hover:bg-blue-50 hover:text-blue-800 transition">
                                ← Anterior
                            </a>
                        @endif

                        {{-- Números de Página --}}
                        @for($i = 1; $i <= $certificadosPendientes->lastPage(); $i++)
                            @if($i == $certificadosPendientes->currentPage())
                                <button disabled class="px-3 py-2 text-sm font-bold text-white bg-blue-600 rounded-lg">
                                    {{ $i }}
                                </button>
                            @else
                                <a href="{{ $certificadosPendientes->url($i) }}" class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-blue-600 transition">
                                    {{ $i }}
                                </a>
                            @endif
                        @endfor
                        ...
                        @if($certificadosPendientes->hasMorePages())
                            <a href="{{ $certificadosPendientes->nextPageUrl() }}" class="px-3 py-2 text-sm font-medium text-blue-600 bg-white border border-gray-300 rounded-lg hover:bg-blue-50 hover:text-blue-800 transition">
                                Siguiente →
                            </a>
                        @else
                            <button disabled class="px-3 py-2 text-sm font-medium text-gray-400 bg-gray-200 rounded-lg cursor-not-allowed">
